    ...
    <div class="result-details landscape:min-w-[480px]">
        ...
        <div class="result-content ml-2">
            ...
            <div class="result-details flex flex-col mb-1 ml-4 pl-2 mt-2 text-sm">
                <span class="px-3 text-primary/90 font-medium text-lg">{{ facilityName }}</span>
                <div class="flex" v-for="(specialty, index) in specialties">
                    <span class="px-3 italic">{{ specialty }}</span>
                </div>
            </div>
            <div class="result-tags flex flex-wrap w-64 mb-6 mt-3 ml-6 pl-2">
...
const specialties = computed(() => {
    ...
    const specialtiesDisplayText = specialties?.map(specialty => {
        const specialtyDisplayText = specialtiesStore.specialtyDisplayOptions.find(
            options => options.code === specialty)?.displayText
