...
const healthcareProfessionalName = computed(
    () => {
        const englishName = resultsStore.$state.activeResult?.professional.names.find(n => n.locale === Locale.EnUs)
        const japaneseName = resultsStore.$state.activeResult?.professional.names.find(n => n.locale === Locale.JaJp)
        ...
        return localStore.locale.code === Locale.EnUs ? englishFullName : japaneseFullName
    }
)
const specialties = computed(
    () => {
        const specialties = resultsStore.$state.activeResult?.professional.specialties
...
const address = computed(() => {
    const addressObj = resultsStore.$state.activeResult?.facilities[0].contact.address
    ...
    return localStore.locale.code === Locale.EnUs ? englishAddress : japaneseAddress
})
const website = computed(() => resultsStore.$state.activeResult?.facilities[0]?.contact?.website)
const phone = computed(() => resultsStore.$state.activeResult?.facilities[0]?.contact?.phone)
</script>
