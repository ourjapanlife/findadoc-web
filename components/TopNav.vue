<template>
    <div
        ...
        <div v-if="$viewport.isGreaterThan('tablet')" class="flex justify-between items-center w-full">
            <div class="flex font-semibold text-xl group transition-colors mr-10">
            ...
            <div class="flex items-center">
                <nav class="flex gap-4 mx-6" v-for="(item, index) in menuStore.menuItems" :key="index">
                    <NuxtLink :to="item.route" class="hover:text-primary-hover transition-colors">{{
            $t(item.displayText) }}
                    </NuxtLink>
                </nav>
                <LocaleSelector />
