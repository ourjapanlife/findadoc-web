<template>
    <div :style="submissionListItemTableColumns" :class="`grid grid-cols-5 p-2`">
        ...
        <div v-if="hasSubmissions" v-for="(submission, index) in modSubmissionsStore.submissionsData" :key="index"
            ...
            <button @click="handleClickToSubmissionForm(submission.id)"
                ...
                <span class="text-start">{{ convertDateToUTC(submission.updatedDate) }}</span>
                <span class="text-start">{{ convertDateToUTC(submission.createdDate) }}</span>
            </button>
        </div>
        <div v-else>
...
const convertDateToUTC = (isoString: string) => {
