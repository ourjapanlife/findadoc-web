<template>
    ...
    >
        <GMarker
            @click="
                searchResultsStore.setActiveSearchResult(
                    location.professional.id
                )
            "
            :options="{
                position: {
                    lat:
                        location.facilities[0]?.mapLatitude ??
                        defaultLocation.lat,
                    lng:
                        location.facilities[0]?.mapLongitude ??
                        defaultLocation.lng,
                },
                icon: markerIcon,
            }"
...
export default defineComponent({
    ...
    setup() {
        ...
        const center = computed(() => {
            const lng =
                useSearchResultsStore().activeResult?.facilities[0]
                    ?.mapLongitude;
            const lat =
                useSearchResultsStore().activeResult?.facilities[0]
                    ?.mapLatitude;
