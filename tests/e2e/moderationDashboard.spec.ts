...
test.describe('Moderation dashboard', () => {
    ...
    test('shows submission type selector and can switch between Submissions, Facilities, Healthcare Professionals', async ({ page }) => {
        ...
        await expect(page.getByRole('button', { name: new RegExp(enUS.modDashboardTopbar.addHealthcareProfessional, 'i') })).toBeVisible()
    ...
    test('shows left nav for submissions with For Review, Approved, Rejected', async ({ page }) => {
